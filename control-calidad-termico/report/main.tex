...
El código completo del análisis (script de regresión, transformación logarítmica, resolución de la EDO), las gráficas generadas, los datos registrados en formato \texttt{.csv} y la bitácora del proceso de modelación se encuentran disponibles en el siguiente repositorio de GitHub:

\begin{center}
    \url{https://github.com/binivazqua/analysis-eqs-difs}
\end{center}

\noindent El repositorio está organizado de la siguiente manera:

\begin{itemize}
    \item \texttt{analysis/} --- script de Python con la regresión lineal y el cálculo de \(k\).
    \item \texttt{documentation/} --- situación problema, datos registrados (\texttt{datos\_registrados.csv}) y bitácora del proceso (\texttt{reporte.md}).
    \item \texttt{output/} --- gráficas generadas (\texttt{regresion\_lineal.png}, \texttt{comparacion\_escenarios.png}) y el reporte compilado.
    \item \texttt{report/} --- código fuente en \LaTeX{} de este documento (\texttt{main.tex}).
\end{itemize}
